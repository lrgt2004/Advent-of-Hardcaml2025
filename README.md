...
### Day10 Part1
Because the minimum number of times in Part1 is required to press each button at most once. Therefore, other lights affected by each button can be converted into binary form first. Then use binary number to simulate DFS, check each bit of the binary number, if it's **1**, XOR the corresponding binary number, finally check whether it is consistent with the result, and take min for the answer.
### Day12
